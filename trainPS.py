...
def train(matFileX, matFileY, matFileTest, modelFile, resFile, \
    ...
    sacFileNameL=[['GS.HYS.20150818.BHE.SAC'],['GS.HYS.20150818.BHN.SAC'],\
            ['GS.HYS.20150818.BHZ.SAC']]
    sacL=obspy.read('GS.HYS.20150818.BH*.SAC')
    badSac=getDataByFileName(sacFileNameL,freq=[2,15])
    badSacData=badSac.data
    print(badSacData.shape)
    ...
    badY=np.zeros((1,2000))
    badN=badSacData.shape[0]
    badCount=0
    badCount1=0
    for i in range(1500):
        j=0
        ...
        for j in range(SCN,SCN+10):
            badCount=int((badCount+1)%len(badL))
        ...
        dataXIn=processX(dataXIn)
        ne =3
        if i >3:
            ne =1
        if i >20 and i%10==0:
            K.set_value(model.optimizer.lr, K.get_value(model.optimizer.lr) * 0.95)
        bs = 50
        if i > 5:
            bs = 75
        if i > 10:
